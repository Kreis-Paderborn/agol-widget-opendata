...
class Proxy {
    ...
    public function getResponse()
    {
        ...
        // $this->proxyLog->log("finished header_list");
    ...
    public function addUuidToUrl($url) {

        $url = str_replace("webatlasde.full_grau", "webatlasde_grau__" . $this->WebAtlasDeUuid, $url);
        $url = str_replace("webatlasde.full", "webatlasde__" . $this->WebAtlasDeUuid, $url);
    ...
    public function setupClassProperties()
    {
        ...
        try {
            ...
            if (!empty($_POST) && empty($_FILES)) { // Is it a POST without files?
                ...
                //AT: Define central variables for treatment of WebAtlasDe

                $this->urlOfProxy = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[PHP_SELF]";

                $this->WebAtlasDeUuid = "2c002aac-be2c-cec2-ae10-79921caab5c3";
            }
    ...
    {
        $this->proxyLog->log("SERVER[SERVER_NAME] :" . $_SERVER['SERVER_NAME']);
        $this->proxyLog->log("SERVER[HTTP_REFERER] :" . $_SERVER['HTTP_REFERER']);
        $this->proxyLog->log("SERVER[REMOTE_ADDR] :" . $_SERVER['REMOTE_ADDR']);
        ...
        $this->proxyLog->log('isUnauthorized: ' . $isUnauthorized);
    ...
    public function initCurl()
    {
        ...
        // AT: Test um SSL-Problem zu beheben laut https://stackoverflow.com/questions/19384161/curl-error-35-unknown-ssl-protocol-error-in-connection-to-api-rkd-reuters-com
    ...
    public function proxyGet($url = null) {
        ...
        // AT: Log url
        $this->proxyLog->log("URL to proxy:" . $url);

        $url = $this->addUuidToUrl($url);
        ...
        try {
            ...
            // AT: Replace licence text
            $this->response = str_replace ( "Die Daten sind lizenzpflichtig." , "© GeoBasis-DE / BKG 2019" , $this->response );

            // AT: Add Proxy to OnlineResouces for https and http
            $this->response = str_replace ( "https://sg.geodatenzentrum.de/" , $this->urlOfProxy . "?https://sg.geodatenzentrum.de/" , $this->response );
            $this->response = str_replace ( "http://sg.geodatenzentrum.de/" , $this->urlOfProxy . "?http://sg.geodatenzentrum.de/" , $this->response );

            // AT: Hide UUID for gray and color
            $this->response = str_replace ( "_grau__" . $this->WebAtlasDeUuid , ".full_grau" , $this->response );            
            $this->response = str_replace ( "__" . $this->WebAtlasDeUuid , ".full" , $this->response );            

            $this->responseClone = $this->response;
            
            $this->contentLength = curl_getinfo($this->ch,CURLINFO_HEADER_SIZE);

            // AT: Log conditions for curl_error
            $this->proxyLog->log("ERROR-NO:" . curl_errno($this->ch));
            $this->proxyLog->log("Response Empty :" . empty($this->response));
